  ...
  describe 'ClassMethods' do
    ...
    describe '.with_query_logging' do
      ...
      it 'handles array results' do
        allow(Time).to receive(:current).and_return(Time.parse('2025-01-01 00:00:00'),
                                                    Time.parse('2025-01-01 00:00:00.030'))
  ...
  describe 'instance methods (callback logging)' do
    let(:test_model) do
      Class.new do
        ...
        def new_record? = @new_record || false
        def persisted? = @persisted || false
        def previously_new_record? = @previously_new_record || false

        def changed_attributes = @changed_attributes ||= {}
  ...
  describe 'integration with ActiveRecord callbacks' do
    let(:user_class) do
      Class.new do
        ...
        def previously_new_record? = false
  ...
  describe 'edge cases and error handling' do
    ...
    describe 'error handling in with_query_logging' do
      let(:test_class) do
        Class.new do
          extend DatabaseLogger::ClassMethods
          def self.name = 'ErrorTestModel'
        end
      end

      it 'handles results that are not arrays or countable' do
        allow(Time).to receive(:current).and_return(Time.parse('2025-01-01 00:00:00'),
                                                    Time.parse('2025-01-01 00:00:00.025'))
      ...
      it 'handles nil results' do
        allow(Time).to receive(:current).and_return(Time.parse('2025-01-01 00:00:00'),
                                                    Time.parse('2025-01-01 00:00:00.015'))
      ...
      it 'handles string results' do
        allow(Time).to receive(:current).and_return(Time.parse('2025-01-01 00:00:00'),
                                                    Time.parse('2025-01-01 00:00:00.035'))
