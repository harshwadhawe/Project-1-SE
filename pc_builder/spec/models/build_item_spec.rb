...
RSpec.describe BuildItem, type: :model do
  ...
  let(:cpu_part) { cpu(price_cents: 30000, wattage: 65) }
  let(:gpu_part) { gpu(price_cents: 50000, wattage: 220) }
  ...
  describe 'total_cost method' do
    it "calculates total cost correctly" do
      ...
      total = bi.total_cost
      expect(total).to eq(60000) # 2 * 30000
    end
    ...
    it "handles nil quantity gracefully (defaults to 1)" do
      ...
      total = bi.total_cost
      expect(total).to eq(30000) # nil quantity treated as 1
    end
